...
const pathVariant = {
  ...
  visible: (i) => ({
    pathLength: 1,
    opacity: 1,
    transition: { duration: 0.9, delay: 0.7 * i },
  }),
};
...
function Header({ isWelcome }) {
  ...
  return (
    <>
      <header>
        <div className='relative mx-auto flex w-11/12 max-w-[24rem] justify-between pt-[clamp(1.3rem,6vw,4rem)] sm:max-w-[48rem] lg:max-w-[90rem]'>
          <div
            onClick={() => navigate('../')}
          ...
          {user === null && (
            <a
              target='_blank'
              rel='noreferrer'
              href='https://github.com/lyn-eva/sutakku'
              className='text-white underline underline-offset-1 duration-200 hover:text-blue-500 leading-[18px]'
            >
              source code
            </a>
          )}
            ...
            <nav>
              ...
              >
                <li onClick={handleRouteChange}>
                  <NavLink
                    className='inline-block w-full border-b-[1px] py-[max(2vw,.5rem)] text-lg active:bg-black'
                    to={`../u/${userInfo?.name}`}
                  >
                    profile
                  </NavLink>
                ...
                <li onClick={handleRouteChange}>
                  <button
                    className='w-full py-[max(2vw,.5rem)] text-lg active:bg-black'
